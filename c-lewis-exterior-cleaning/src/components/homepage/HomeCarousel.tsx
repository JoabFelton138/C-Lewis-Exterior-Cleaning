             ...
             <picture>
              ...
              <div className="absolute inset-0 flex items-center justify-center px-2 sm:px-4 md:px-6">
                <div className="flex flex-col items-center gap-2 sm:gap-4 md:gap-6 px-2 sm:px-4">
                  <h2
                    className={`text-lg sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl whitespace-nowrap title-style text-white text-center animate-slide-down drop-shadow-2xl ${activeSlide === index && !isTransitioning ? "active" : ""}`}
                  >
                    {slide.title}
                  </h2>
                  <h4
                    className={`text-xs sm:text-lg md:text-xl lg:text-2xl title-style text-white/90 text-center animate-slide-left drop-shadow-lg line-clamp-2 sm:line-clamp-none max-w-[320px] sm:max-w-[450px] md:max-w-[550px] lg:max-w-[650px] xl:max-w-[750px] ${activeSlide === index && !isTransitioning ? "active" : ""}`}
                  >
                    {slide.subtitle}
                  </h4>
                  <Button
